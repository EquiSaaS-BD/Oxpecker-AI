            ...
            {["All", "Ready", "Pending", "Delivered"].map(status => (
              <button
                ...
                className={`h-[40px] px-4 rounded-xl text-[13px] font-bold whitespace-nowrap transition-colors border ${
                  filterStatus === status ? "bg-[#111827] border-[#111827] text-white" : "bg-white border-[#E5E7EB] text-[#6B7280] hover:bg-slate-50"
                }`}
              >
                {status}
              </button>
          ...
          {filteredReports.map((report) => (
            <div key={report.id} className="bg-white p-5 flex flex-col gap-3">
              ...
              <button 
                onClick={() => setSelectedReportId(report.id)}
                disabled={report.status === "Pending"}
                className="w-full h-[48px] mt-1 bg-blue-50 text-[#2F80ED] font-bold text-[15px] rounded-xl flex items-center justify-center gap-2 transition-transform active:scale-95 disabled:opacity-50 disabled:bg-slate-100 disabled:text-slate-400"
              >

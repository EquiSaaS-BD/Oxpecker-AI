...
export default function ResetPasswordPage() {
  ...
  const verifyOtp = async () => {
    ...
    const isValid = !verifyErr && verifyData?.session;
        ...
        <div className="flex items-center gap-3 mb-8 w-full max-w-[280px]">
          {[1, 2, 3].map((s) => (
            <div key={s} className="flex-1 flex items-center">
              <div className={`h-2 flex-1 rounded-full transition-all duration-500 ${step >= s ? "bg-sky-600" : "bg-slate-100"}`} />
            </div>
          ))}
        </div>
          ...
          <AnimatePresence mode="wait">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full h-[52px] bg-transparent border-2 border-slate-200 rounded-xl py-3 pl-12 pr-4 text-slate-800 focus:outline-none focus:border-sky-500 hover:border-slate-300 transition-all"
                        required
                      />
                    </div>
                  </div>
                  
                  <button type="submit" disabled={isLoading} className="w-full h-[52px] bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-bold transition-all shadow-md hover:-translate-y-0.5 flex items-center justify-center gap-2">
            ...
            {step === 2 && (
              <motion.div key="step2" variants={slideVariants} initial="initial" animate="animate" exit="exit" className="w-full">
                <div className="text-center mb-8">
                  <div className="w-16 h-16 bg-sky-50 border border-sky-200 rounded-full flex items-center justify-center mx-auto mb-4">
                    <KeyRound size={32} className="text-sky-600" />
                  </div>
                  ...
                  <div className="flex justify-center gap-2 sm:gap-3">
                    {otp.map((digit, i) => (
                      <input
                        ...
                        className="w-[46px] sm:w-[54px] h-[64px] text-center text-2xl font-extrabold text-slate-800 bg-transparent border-2 border-slate-200 rounded-xl focus:outline-none focus:border-sky-500 focus:bg-sky-50/30 hover:border-slate-300 transition-all"
                      />
                    ))}
                  </div>
                  
                  <div className="flex flex-col gap-4">
                    <button onClick={verifyOtp} disabled={isLoading} className="w-full h-[52px] bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-bold transition-all shadow-md hover:-translate-y-0.5 flex items-center justify-center gap-2">
                      {isLoading ? <span className="animate-spin rounded-full h-6 w-6 border-b-2 border-white"></span> : "Verify OTP"}
                    </button>
                    <p className="text-center text-sm font-semibold text-slate-500">
                      Didn't receive code?{" "}
                      <button onClick={handleResendOtp} disabled={isResending} className="text-sky-600 hover:underline ml-1 font-bold">
              ...
              <motion.div key="step3" variants={slideVariants} initial="initial" animate="animate" exit="exit" className="w-full">
                ...
                <form onSubmit={handleResetPassword} className="space-y-4">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <div className="relative">
                      <CheckCircle2 className={`absolute left-4 top-1/2 -translate-y-1/2 ${confirmPassword && newPassword === confirmPassword ? 'text-emerald-500' : 'text-slate-500'}`} size={20} />
                      <input 
                        type="password" 

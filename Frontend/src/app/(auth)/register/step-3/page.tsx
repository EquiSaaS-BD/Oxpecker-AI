...
<div className="w-8 h-8 rounded-full bg-primary text-on-primary flex items-center justify-center font-label-md font-bold transition-colors">২</div>
<span className="font-label-md text-xs text-primary">তথ্য</span>
</div>
{/*  Step 3 Indicator  */}
<div className="relative z-10 flex flex-col items-center gap-xs" id="stepper-3">
<div className="w-8 h-8 rounded-full bg-primary text-on-primary flex items-center justify-center font-label-md font-bold transition-colors shadow-md">৩</div>
<span className="font-label-md text-xs text-primary">নিরাপত্তা</span>
</div>
</div>
</div>
<form action="#" className="flex-grow flex flex-col" id="registration-form" method="POST">
{/*  Step 1: Role Selection  */}
<div className="flex-grow space-y-md step-hidden" id="step-1">
<div>
...
<div className="flex-grow space-y-md" id="step-3"><div><h2 className="font-headline-sm text-headline-sm text-primary mb-xs">৩. নিরাপত্তা</h2><p className="font-body-sm text-on-surface-variant mb-lg">Authentication Setup</p><div className="grid grid-cols-1 md:grid-cols-2 gap-lg mb-lg"><div className="space-y-sm"><label className="font-label-md text-label-md text-on-surface-variant block">পাসওয়ার্ড <span className="text-error">*</span></label><div className="relative"><input className="w-full bg-surface border border-outline-variant rounded-md px-md py-sm font-body-md text-on-surface focus:border-primary focus:ring-1 focus:ring-primary transition-shadow" placeholder="••••••••" type="password"/><span className="material-symbols-outlined absolute right-3 top-2.5 text-outline cursor-pointer hover:text-primary">visibility</span></div></div><div className="space-y-sm"><label className="font-label-md text-label-md text-on-surface-variant block">পাসওয়ার্ড নিশ্চিত করুন <span className="text-error">*</span></label><div className="relative"><input className="w-full bg-surface border border-outline-variant rounded-md px-md py-sm font-body-md text-on-surface focus:border-primary focus:ring-1 focus:ring-primary transition-shadow" placeholder="••••••••" type="password"/><span className="material-symbols-outlined absolute right-3 top-2.5 text-outline cursor-pointer hover:text-primary">visibility</span></div></div><div className="space-y-sm md:col-span-2"><label className="font-label-md text-label-md text-on-surface-variant block">রেফারেল কোড (ঐচ্ছিক)</label><input className="w-full bg-surface border border-outline-variant rounded-md px-md py-sm font-body-md text-on-surface focus:border-primary focus:ring-1 focus:ring-primary transition-shadow" placeholder="Enter Code" type="text"/></div></div><div className="flex items-start gap-sm mb-lg"><input className="mt-1 rounded border-outline-variant text-primary focus:ring-primary" id="terms" type="checkbox"/><label className="font-body-sm text-on-surface-variant leading-tight" htmlFor="terms">আমি <a className="text-primary hover:underline" href="#">শর্তাবলী</a> এবং <a className="text-primary hover:underline" href="#">গোপনীয়তা নীতি</a> এর সাথে একমত। <br/><span className="text-[10px] opacity-70">I agree to the Terms of Service and Privacy Policy.</span></label></div></div><div className="mt-auto pt-xl flex flex-col md:flex-row gap-md items-center justify-between"><button className="px-lg py-sm border border-outline-variant text-on-surface rounded-md font-label-md text-label-md hover:bg-surface-container transition-colors flex items-center justify-center gap-xs" onClick={() => {}} type="button"><span className="material-symbols-outlined text-sm">arrow_back</span><span>পেছনে (Back)</span></button><div className="flex flex-col md:flex-row items-center gap-md"><button className="w-full md:w-auto px-xl py-sm bg-primary text-on-primary rounded-md font-label-md text-label-md hover:bg-primary-fixed-variant transition-colors flex items-center justify-center gap-xs shadow-md" type="button"><span>অ্যাকাউন্ট তৈরি করুন</span><span className="material-symbols-outlined text-sm">check_circle</span></button></div></div></div>
</form>
</div>
</div>
</main>

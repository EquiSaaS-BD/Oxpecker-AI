...
export default function ProfileSettingsPage() {
  ...
  // --- Load Initial State ---
  useEffect(() => {
    const savedIdStatus = localStorage.getItem('oxpecker_doc_id_status');
    if (savedIdStatus) setIdStatus(savedIdStatus as any);
    
    const savedIdType = localStorage.getItem('oxpecker_doc_id_type');
      ...
      try {
        ...
        if (prof) {
          setFormData(prev => ({
            ...
            specialization: prof.specialty || prev.specialization,
            gender: prof.gender || prev.gender,
            avatar: prof.image || prev.avatar
          }));
        }
      } catch {}
    }
  ...
  const handleSubmitID = () => {
    ...
    setTimeout(() => {
      ...
      setIdStatus("pending");
      localStorage.setItem('oxpecker_doc_id_status', 'pending');
      localStorage.setItem('oxpecker_doc_id_type', idType);
    }, 2000);
  };
  ...
  return (
    <div className="space-y-8 animate-fade-in-up pb-20">
      ...
      <div className="bg-white rounded-[24px] shadow-none border border-slate-300 p-6 lg:p-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <h1 className="text-2xl font-extrabold text-slate-700 tracking-tight flex items-center gap-3">
              Public Profile 
          ...
          <div className="w-full md:w-[350px]">
            <div className="flex justify-between items-end mb-2">
              <span className="text-[13px] font-bold text-slate-500 uppercase tracking-wider">Profile Verification</span>
            ...
            {isGreen && <p className="text-[12px] font-bold text-emerald-500 mt-2 flex items-center gap-1 justify-end"><CheckCircle2 size={14}/> Identity Verified</p>}
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-8">
        ...
        <div className="bg-white rounded-[24px] shadow-none border border-slate-300 overflow-hidden w-full">
          
          <div className="p-6 border-b border-slate-200 flex flex-col sm:flex-row items-center gap-6 bg-slate-50">
            <div className="relative group shrink-0">
              <div className="w-[100px] h-[100px] rounded-full overflow-hidden border-4 border-white shadow-none bg-slate-800 relative">
            ...
            <div className="text-center sm:text-left flex-1">
              <h3 className="text-[18px] font-bold text-slate-700 flex items-center justify-center sm:justify-start gap-2">
                Public Photo 
              </h3>
              <p className="text-[13px] text-slate-500 mt-1 mb-4">Recommended size: 400x400px (JPG or PNG).</p>
              <div className="flex items-center justify-center sm:justify-start gap-3">
                <ImageUploadCropper 
                  onUpload={(base64) => setFormData(prev => ({ ...prev, avatar: base64 }))}
          ...
          <div className="p-6 lg:p-8 space-y-6">
            {/* Basic Info */}
            <div>
              <h4 className="text-[15px] font-bold text-slate-700 mb-4 flex items-center gap-2">
                <User size={18} className="text-[#2F80ED]" /> Basic Information
              </h4>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div className="md:col-span-1">
                  <label className="block text-[13px] font-semibold text-slate-500 mb-1.5 ml-1">Full Name</label>
                  <input 
                    type="text" 
                    value={formData.name}
                    onChange={(e) => setFormData({...formData, name: e.target.value})}
                    className="w-full h-[48px] bg-slate-50 border border-slate-300 rounded-xl px-4 text-[14px] text-slate-700 focus:outline-none focus:border-[#2F80ED] focus:bg-white transition-all"
                  />
                </div>
                
                <div className="md:col-span-1">
                  <label className="block text-[13px] font-semibold text-slate-500 mb-1.5 ml-1">Gender</label>
                  <select 
                ...
                <div className="md:col-span-1">
                  <label className="block text-[13px] font-semibold text-slate-500 mb-1.5 ml-1">Specialization</label>
                  <select 
                ...
                <div className="md:col-span-3">
                  <label className="block text-[13px] font-semibold text-slate-500 mb-1.5 ml-1">Professional Bio</label>
                  <textarea 
                    value={formData.bio}
                    onChange={(e) => setFormData({...formData, bio: e.target.value})}
                    className="w-full min-h-[100px] bg-slate-50 border border-slate-300 rounded-xl p-4 text-[14px] text-slate-700 focus:outline-none focus:border-[#2F80ED] focus:bg-white transition-all resize-none"
                  />
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  <label className="block text-[13px] font-semibold text-slate-500 mb-1.5 ml-1">Consultation Fee (BDT)</label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="block text-[13px] font-semibold text-slate-500 mb-1.5 ml-1 flex items-center gap-1.5">
                      Public Phone Number
                  </label>
                  <input 
                    type="text" 
            ...
            <div className="pt-4 flex justify-end">
              <button 
                ...
                disabled={isSaving}
                className="w-full sm:w-auto px-8 py-3 bg-slate-900 hover:bg-slate-800 text-white text-[14px] font-bold rounded-xl shadow-sm transition-all flex items-center justify-center gap-2"
              >
                {isSaving ? (
        ...
        <div className="bg-white rounded-[24px] shadow-none border border-slate-300 overflow-hidden w-full xl:w-1/2">
          <div className="p-6 border-b border-slate-200 bg-[#8B5CF6]/5">
            <h3 className="text-[16px] font-bold text-slate-700 flex items-center gap-2">
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-[13px] font-semibold text-slate-500 mb-1.5 flex justify-between items-center">
                  <span>Document Type</span>
                  {idStatus === "pending" && <Lock size={12} className="text-slate-500" />}
                </label>
                <select 
                  value={idType}
                  onChange={(e) => setIdType(e.target.value)}
                  disabled={idStatus === "pending"}
                  className="w-full h-[48px] bg-slate-50 border border-slate-300 rounded-xl px-4 text-[14px] text-slate-700 focus:outline-none focus:border-[#8B5CF6] disabled:opacity-60 disabled:cursor-not-allowed transition-all"
                >
              ...
              <div>
                ...
                <ImageUploadCropper 
                  onUpload={(base64) => setFormData(prev => ({ ...prev, verificationDocument: base64 }))}
                ...
                <div 
                  ...
                  className={`w-full border-2 border-dashed rounded-[16px] p-6 flex flex-col items-center justify-center transition-all ${
                    idStatus === "pending" 
                      ...
                      : "border-slate-600 bg-slate-50 hover:bg-[#8B5CF6]/5 hover:border-[#8B5CF6] cursor-pointer"
                  }`}
                >
                  {idFile ? (
                    <div className="flex flex-col items-center">
                      <FileText size={32} className="text-[#22C55E] mb-2" />
                      <span className="text-[13px] font-bold text-slate-600 text-center">{idFile.name}</span>
                      <span className="text-[11px] text-slate-500 mt-1">Ready to upload</span>
                    </div>
                  ) : (
                    <div className="flex flex-col items-center">
                      <UploadCloud size={32} className="text-slate-500 mb-3" />
                      <span className="text-[14px] font-bold text-slate-600">Click to upload document</span>
            ...
            {idStatus !== "pending" && (
              <button 
                ...
                className="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-3.5 rounded-xl shadow-sm transition-all active:scale-[0.98] flex items-center justify-center gap-2"
              >
                <IdCard size={18} />

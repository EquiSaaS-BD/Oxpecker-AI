...
export default function AppLayout({
  ...
  const isPublicDiscoveryRoute = 
    ...
    pathname === "/hospitals" || pathname.startsWith("/hospitals/") ||
    pathname === "/nutrition" || pathname.startsWith("/nutrition/");

  if (isPublicDiscoveryRoute) {
    return (
      <div className="min-h-screen bg-[#F8FAFC] flex flex-col selection:bg-slate-200 selection:text-slate-900">
        <Navbar />
          ...
          <div className="flex-1 flex flex-col h-full overflow-hidden relative">
            ...
            {!isSpecialRoute && (
              <header className="lg:hidden absolute top-0 left-0 right-0 h-14 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-4 z-[60] shadow-sm">
                ...
                {/* Clickable Logo */}
                <Link href="/?view=home" className="flex items-center gap-2 font-[900] text-slate-800 text-lg tracking-tight hover:opacity-90 transition-opacity">
            ...
            {/* Desktop Top-Right Profile (Visible >= 2xl) */}
            {!isSpecialRoute && (
              <div className="hidden 2xl:flex absolute top-4 right-6 z-[60]">
                <Link 
                  href="/settings"
                  className="w-10 h-10 rounded-full bg-sky-600 text-white flex items-center justify-center text-sm font-black overflow-hidden border-2 border-white shadow-md hover:scale-105 transition-transform relative"
                  title={user?.name || "User Profile"}
                >
                  {user?.image ? (
                    <Image src={user.image} alt={user?.name || "User"} fill sizes="40px" className="object-cover" />
                  ) : (
                    user?.name ? user.name.charAt(0).toUpperCase() : 'U'
                  )}
                </Link>

  ...
  const loadAdminProfile = useCallback(async () => {
    ...
    try {
      ...
      setFormData({
        ...
        email: prof.email || user?.email || "admin@oxpecker.equisaas-bd.com",
  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      <div className="max-w-3xl bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm">
        {/* Banner */}
        <div className="bg-white px-8 py-10 text-slate-900 flex flex-col sm:flex-row items-center gap-6 relative">
          <div className="relative group">
            <div className="w-24 h-24 rounded-full bg-slate-800 flex items-center justify-center overflow-hidden border-2 border-white/50 shadow-lg relative">
              {imagePreview ? (
                ...
                <span className="text-3xl font-bold text-slate-900">{(formData.name || "S").charAt(0).toUpperCase()}</span>
              )}
            </div>
            <label
          ...
          <div className="text-center sm:text-left">
            ...
            <p className="text-slate-600 text-sm mt-1 font-medium">{formData.title}</p>
            <p className="text-xs text-slate-500 mt-1 font-mono">{formData.email}</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSave} className="p-8 space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <div className="relative">
                ...
                />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-700">
          ...
          <div className="pt-2 flex justify-end">
            <button
              type="submit"
              disabled={isSaving}
              className="flex items-center gap-2 px-8 h-12 bg-white hover:bg-slate-50 text-slate-900 font-bold rounded-xl transition-all shadow-md text-sm disabled:opacity-70"
            >
              {isSaving ? (

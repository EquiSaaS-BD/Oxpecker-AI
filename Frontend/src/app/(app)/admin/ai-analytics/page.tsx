...
import {
  ...
  Brain, ChevronDown, AlertTriangle,
} from "lucide-react";
...
const LANGUAGES = [
  { lang: "Bangla", percentage: 62, code: "BN" },
  { lang: "English", percentage: 31, code: "EN" },
  { lang: "Hindi", percentage: 5, code: "HI" },
  { lang: "Arabic", percentage: 2, code: "AR" },
];
          ...
          <div className="flex items-end gap-3 h-[200px]">
            {WEEKLY_DATA.map((d, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-2">
                ...
                />
                <span className="text-[12px] font-medium text-slate-500">{d.day}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-4">
            {PROVIDER_USAGE.map((p, i) => (
              <div key={i}>
                ...
                <p className="text-[11px] text-slate-500 mt-1">{p.requests.toLocaleString()} requests</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6">
          ...
          <div className="space-y-4">
            {LANGUAGES.map((l, i) => (
              <div key={i} className="flex items-center gap-4">
                <span className="w-10 h-10 rounded-xl bg-slate-100 border border-slate-200 flex items-center justify-center text-xs font-black text-slate-700">{l.code}</span>
          ...
          <div className="mt-8 p-4 bg-rose-50 border border-rose-100 rounded-xl">
            <h4 className="text-[14px] font-bold text-rose-700 mb-3 flex items-center gap-2">
              <AlertTriangle size={16} className="text-rose-600" /> Emergency Detections
            </h4>

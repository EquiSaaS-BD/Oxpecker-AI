  ...
  if (isSuccess) {
    return (
      <div className="h-full flex items-center justify-center bg-[#F8FAFC] p-6">
        ...
        >
          <div className="w-24 h-24 bg-sky-600/10 rounded-full flex items-center justify-center mx-auto mb-6">
            <CheckCircle2 size={48} className="text-sky-600" />
          </div>
          ...
          <div className="space-y-3">
            <Link href="/appointments" className="flex items-center justify-center w-full h-14 bg-sky-600 text-white rounded-[16px] font-bold text-[16px] shadow-lg shadow-primary/20 hover:bg-sky-600/90 transition-colors">
              View Digital Memo
            </Link>
      ...
      <div className="max-w-[1200px] mx-auto px-4 md:px-6 pt-8">
        ...
        <div className="mb-6">
          ...
          <div className="flex flex-wrap items-center gap-3 md:gap-4 text-[14px] md:text-[15px] font-bold text-slate-600">
            ...
            <span className="text-sky-600">{bed.hospital}</span>
            <span className="w-1 h-1 rounded-full bg-slate-300" />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {bed.features.map((feature, i) => (
                  <div key={i} className="flex items-center gap-4">
                    <feature.icon size={26} className="text-sky-600/70" />
          ...
          <div className="w-full lg:w-[420px] shrink-0">
            <div className="sticky top-[100px] bg-white rounded-[24px] p-6 md:p-8 shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-slate-100">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="text" name="patientName" value={formData.patientName} onChange={handleFormChange} placeholder="Enter full name" className="w-full h-12 pl-10 pr-4 bg-slate-50 border border-slate-100 rounded-[12px] text-[14px] font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary/20 placeholder:text-slate-500" />
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3">
                    <div className="w-1/4">
                      ...
                      <input type="number" name="age" value={formData.age} onChange={handleFormChange} placeholder="Age" className="w-full h-12 px-3 bg-slate-50 border border-slate-100 rounded-[12px] text-[14px] font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary/20 placeholder:text-slate-500" />
                    </div>
                    
                    <div className="w-1/3">
                      <label className="block text-[12px] font-bold text-slate-700 mb-1.5 ml-1">Gender <span className="text-rose-500">*</span></label>
                      <select name="gender" value={formData.gender} onChange={(e: any) => handleFormChange(e)} className={`w-full h-12 px-3 bg-slate-50 border border-slate-100 rounded-[12px] text-[14px] font-medium focus:outline-none focus:ring-2 focus:ring-primary/20 ${formData.gender ? 'text-slate-800' : 'text-slate-500'}`}>
                    ...
                    <div className="w-full">
                      ...
                      <div className="relative">
                        ...
                        <input type="text" name="guardianName" value={formData.guardianName} onChange={handleFormChange} placeholder="Guardian name" className="w-full h-12 pl-10 pr-4 bg-slate-50 border border-slate-100 rounded-[12px] text-[14px] font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary/20 placeholder:text-slate-500" />
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="text" name="address" value={formData.address} onChange={handleFormChange} placeholder="Full address" className="w-full h-12 pl-10 pr-4 bg-slate-50 border border-slate-100 rounded-[12px] text-[14px] font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary/20 placeholder:text-slate-500" />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="tel" name="phone" value={formData.phone} onChange={handleFormChange} placeholder="Phone number" className="w-full h-12 pl-10 pr-4 bg-slate-50 border border-slate-100 rounded-[12px] text-[14px] font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary/20 placeholder:text-slate-500" />
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                ...
                <label className={`flex flex-col items-center justify-center gap-2 p-4 rounded-[12px] cursor-pointer transition-all ${paymentMethod === 'card' ? 'bg-sky-600/10 ring-2 ring-primary' : 'bg-slate-50 hover:bg-slate-100'}`}>
                  <input type="radio" name="payment" onChange={() => setPaymentMethod('card')} className="hidden" />
                  <div className="w-10 h-10 bg-white rounded-lg shadow-sm flex items-center justify-center text-sky-600">
                    <CreditCard size={20} />
                  </div>
              ...
              <div className="bg-slate-50 rounded-[12px] p-4 mb-6 space-y-2 border border-slate-100">
                ...
                <div className="flex justify-between items-center text-[16px] font-[800] text-slate-900">
                  ...
                  <span className="text-sky-600">৳{bed.advanceRequired}</span>
                </div>
              </div>
              ...
              <button 
                ...
                className="w-full h-14 bg-sky-600 hover:bg-sky-600/90 text-white rounded-[12px] font-bold text-[16px] transition-all shadow-lg shadow-primary/20 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                <ShieldCheck size={20} /> Pay ৳{bed.advanceRequired} to Confirm
              </button>

              <div className="text-center text-[12px] text-slate-500 font-medium mt-4">

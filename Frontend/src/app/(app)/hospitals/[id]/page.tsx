...
export default function HospitalProfile() {
  ...
  return (
    <div className="h-full overflow-y-auto bg-slate-50 font-sans pb-24 relative scroll-smooth">
      ...
      <div className="relative w-full h-[280px] md:h-[340px] bg-white">
        ...
        <div className="absolute bottom-0 left-0 w-full p-4 md:p-8 flex items-end justify-between max-w-[1200px] mx-auto">
          <div className="flex flex-col md:flex-row md:items-end gap-4 md:gap-6">
            ...
            <motion.div 
              ...
              className="text-slate-900 mb-1 md:mb-2"
            >
              <h1 className="text-2xl md:text-4xl font-bold leading-tight mb-2 tracking-tight">
                {hospital.name}
              </h1>
              <div className="flex flex-wrap items-center gap-3 text-sm md:text-base text-slate-700 font-medium">
          ...
          >
            <button className="w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-md flex items-center justify-center text-slate-900 transition-colors">
              <Share2 size={20} />
            </button>
            <button className="w-12 h-12 rounded-full bg-sky-600 hover:bg-sky-600/90 flex items-center justify-center text-white shadow-md transition-colors">
              <Phone size={20} />
            </button>
          </motion.div>
        </div>
      ...
      <div className="max-w-[1200px] mx-auto px-4 md:px-6 lg:px-8 py-8 space-y-12">
        ...
        >
          {[
            ...
            <motion.div key={i} variants={itemVariants} className="bg-white rounded-2xl p-5 shadow-sm flex flex-col justify-center items-center text-center hover:shadow-md transition-shadow">
              <div className="w-10 h-10 bg-sky-600/5 rounded-full flex items-center justify-center mb-2">
              ...
              >
                ...
                <div className="p-5 flex flex-col flex-1">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="bg-sky-600/5 text-sky-600 px-2 py-1 rounded text-xs font-bold">
                      {bed.available} Left
                    </div>
                  </div>
                  ...
                  <div className="mt-auto flex items-center justify-between pt-4 border-t border-slate-50">
                    ...
                    <Link href={`/hospitals/${hospital.id}/beds/${bed.id}`} className="px-5 py-2 bg-[#22C55E] hover:bg-[#6DDA6E] text-slate-900 rounded-xl text-sm font-semibold transition-colors">
                      Book Room
                    </Link>
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          >
            <div className="mb-8">
              <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3">Key Specialties</h3>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {hospital.insurance.map(ins => (
                  <span key={ins} className="bg-sky-600/5 text-sky-600 px-3 py-1.5 rounded-lg text-sm font-bold">{ins}</span>
                ))}
              </div>
            </div>
          </motion.div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
            {hospital.facilities.map((fac, i) => (
              <div key={i} className="bg-white rounded-2xl p-4 flex flex-col items-center justify-center hover:shadow-md transition-shadow shadow-sm">
                <fac.icon size={24} className="text-sky-600 mb-2" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {hospital.doctors.map((doc, i) => (
              <Link key={i} href={`/doctors/${doc.id}`} className="bg-white rounded-2xl p-5 shadow-sm flex items-center gap-4 group cursor-pointer hover:shadow-md transition-all">
                ...
                <div>
                  <h3 className="text-[15px] font-bold text-slate-900 group-hover:text-sky-600 transition-colors">{doc.name}</h3>
                  <p className="text-[13px] text-slate-500 font-medium mb-1">{doc.spec}</p>
                  <p className="text-[12px] font-bold text-slate-500">{doc.exp}</p>
                </div>
              </Link>
            ))}
          </div>
      ...
      <AnimatePresence>
        {galleryOpen !== null && (
          ...
          >
            <button onClick={() => setGalleryOpen(null)} className="absolute top-6 right-6 text-slate-900 p-2 hover:bg-white/20 rounded-full transition-colors">
              <X size={28} />
            </button>

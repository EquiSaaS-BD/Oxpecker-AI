...
export default function SavedItemsPage() {
  const [savedItems, setSavedItems] = useState(initialMockSaved);
  const [activeTab, setActiveTab] = useState("All");
  const [searchQuery, setSearchQuery] = useState("");

  const handleRemove = (id: number) => {
    setSavedItems(prev => prev.filter(item => item.id !== id));
    toast.info("আইটেমটি বুকমার্ক থেকে সরানো হয়েছে");
  };
        ...
        <div className="max-w-[1200px] mx-auto flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h1 className="text-[28px] md:text-[32px] font-[800] text-slate-900 leading-tight flex items-center gap-3">
              <Bookmark className="text-sky-600 fill-primary/10" size={32} /> Saved Items
            </h1>
          ...
          <div className="relative w-full md:w-[350px]">
            ...
            <input 
              ...
              className="w-full h-12 pl-10 pr-4 bg-slate-50 border border-slate-200 rounded-[14px] text-[15px] font-medium text-slate-800 placeholder:text-slate-500 focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all shadow-inner"
            />
          </div>
        </div>
            ...
            {filteredItems.map(item => (
              <div key={item.id} className="bg-white rounded-[20px] overflow-hidden border border-slate-200 shadow-sm hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] hover:-translate-y-1 hover:border-primary/30 transition-all duration-300 flex flex-col group">
                ...
                <div className="h-[180px] w-full bg-slate-100 relative overflow-hidden">
                  ...
                  <button onClick={() => handleRemove(item.id)} className="absolute top-3 right-3 w-8 h-8 bg-white/90 backdrop-blur-md rounded-full flex items-center justify-center text-rose-500 hover:bg-rose-500 hover:text-white transition-colors shadow-sm">
                    <Trash2 size={14} />
                  </button>
                </div>
                ...
                <div className="p-5 flex flex-col flex-1">
                  <h3 className="text-[18px] font-bold text-slate-900 leading-tight mb-1 group-hover:text-sky-600 transition-colors line-clamp-1">{item.name}</h3>
                  ...
                  <div className="mt-auto flex items-center justify-between">
                    ...
                    {item.readTime && (
                      <span className="text-[12px] font-bold text-sky-600 bg-sky-600/10 px-2 py-1 rounded-md">{item.readTime}</span>
                    )}
                    
                    <Link 
                      href={getItemLink(item.type)}
                      className="h-9 px-4 bg-slate-50 hover:bg-sky-600 text-slate-600 hover:text-white rounded-xl font-bold text-[13px] transition-colors flex items-center gap-1.5 ml-auto"
                    >

...
import { 
  Pill, AlertTriangle, ShieldAlert, Bot, CheckCircle2, ChevronRight,
    ...
    <div className="min-h-screen bg-[#F8FAFC] font-sans pb-[100px] md:pb-12">
      ...
      <div className="bg-white border-b border-[#E5E7EB] px-4 md:px-8 py-4 sticky top-0 z-50">
        <div className="max-w-[1280px] mx-auto flex items-center gap-2 text-sm text-slate-500 font-medium">
          <Link href="/medicines" className="hover:text-sky-600 transition-colors">Medicines</Link>
          <ChevronRight size={14} />
          <Link href="/medicines" className="hover:text-sky-600 transition-colors">{medicine.category}</Link>
          <ChevronRight size={14} />
      ...
      <div className="max-w-[1280px] mx-auto px-4 md:px-6 lg:px-8 pt-8 flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex-1 flex flex-col">
              <div className="flex justify-between items-start mb-2">
                ...
                <div className="hidden sm:flex gap-2">
                  ...
                  <button className="w-10 h-10 rounded-full border border-slate-200 flex items-center justify-center text-slate-500 hover:text-sky-600 hover:border-primary/30 transition-colors">
                    <Share2 size={18} />
                  </button>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                ...
                <span className="h-[34px] px-3 bg-sky-600/10 text-sky-600 font-bold text-sm rounded-lg flex items-center border border-primary/20">
                  {medicine.strength}
                </span>
              ...
              <div className="mt-auto flex justify-between items-end">
                ...
                <div className="lg:hidden text-right">
                  ...
                  <div className="text-xs font-bold text-sky-600">{medicine.availability}</div>
                </div>
              </div>
            </div>
          </motion.section>
          ...
          >
            ...
            <div className="relative z-10">
              <h2 className="text-[20px] font-bold text-[#065f46] mb-4 flex items-center gap-2">
                <Bot size={20} className="text-[#059669]" /> AI Analysis Summary
              </h2>
                ...
                <ul className="space-y-2">
                  {medicine.benefits.map((ben, i) => (
                    <li key={i} className="flex gap-2 items-start text-[15px] text-slate-600 font-medium">
                      <CheckCircle2 size={18} className="text-sky-600 mt-0.5 shrink-0" /> {ben}
                    </li>
                  ))}
                </ul>
              </div>
              ...
              {[
                ...
                <div key={i} className="flex gap-4 p-4 rounded-[16px] bg-slate-50 border border-slate-100 h-auto sm:h-[110px]">
                  <div className="w-10 h-10 rounded-full bg-white shadow-sm flex items-center justify-center shrink-0">
                    <safe.icon size={20} className={safe.data.status === 'Safe' ? 'text-sky-600' : safe.data.status === 'Consult Doctor' ? 'text-amber-500' : 'text-rose-500'} />
                  </div>
                  <div>
            ...
            <div className="space-y-3">
              {(activeInteractionTab === 'food' ? medicine.interactions.food : medicine.interactions.drug).map((int, i) => (
                <div key={i} className="flex gap-4 p-4 rounded-[12px] bg-slate-50 border border-slate-100 items-center">
                  <ShieldAlert size={24} className={int.risk === 'High' ? 'text-rose-500' : int.risk === 'Moderate' ? 'text-amber-500' : 'text-slate-500'} shrink-0 />
              ...
              <table className="w-full text-left border-collapse min-w-[600px]">
                <thead>
                  <tr className="border-b border-slate-200">
                    <th className="py-3 px-4 text-[12px] font-bold text-slate-500 uppercase tracking-wider">Medicine Name</th>
                    <th className="py-3 px-4 text-[12px] font-bold text-slate-500 uppercase tracking-wider">Manufacturer</th>
                    <th className="py-3 px-4 text-[12px] font-bold text-slate-500 uppercase tracking-wider">Price/Strip</th>
                    <th className="py-3 px-4 text-[12px] font-bold text-slate-500 uppercase tracking-wider text-right">Action</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  <tr className="h-[60px] bg-sky-600/5">
                  ...
                  {medicine.alternatives.map((alt, i) => (
                    <tr key={i} className="h-[60px] hover:bg-slate-50 transition-colors">
                      ...
                      <td className="py-3 px-4 text-right">
                        <button className="px-4 py-1.5 border border-slate-200 rounded-lg text-[13px] font-bold text-slate-600 hover:border-primary hover:text-sky-600 transition-colors">Compare</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
                ...
                <div key={i} className="bg-white rounded-[18px] p-6 h-[160px] shadow-sm border border-[#E5E7EB] flex flex-col">
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 rounded-full bg-white text-slate-900 flex items-center justify-center font-bold text-xs">{rev.name.charAt(0)}</div>
                      <div>
                        <h4 className="text-[14px] font-bold text-slate-900 leading-tight">{rev.name}</h4>
                        <span className="text-[11px] text-slate-500 font-medium">{rev.date}</span>
                      </div>
                    </div>
        ...
        <div className="hidden lg:block w-[340px] shrink-0">
          <div className="sticky top-[100px] bg-white rounded-[18px] p-6 shadow-md border border-[#E5E7EB]">
            ...
            <p className="text-[14px] font-bold text-sky-600 mb-6 flex items-center gap-2">
              <CheckCircle2 size={16} /> {medicine.availability}
            </p>

            <div className="space-y-3 mb-6 border-y border-slate-100 py-6">
              <button className="w-full h-[52px] bg-sky-600 hover:bg-[#5bc95c] text-white rounded-[14px] font-bold text-[16px] transition-colors flex items-center justify-center gap-2 shadow-lg shadow-primary/20">
                <ShoppingCart size={20} /> Add to Cart
              </button>
              <button className="w-full h-[52px] bg-white hover:bg-slate-50 text-slate-900 rounded-[14px] font-bold text-[16px] transition-colors flex items-center justify-center gap-2">
      ...
      <div className="lg:hidden fixed bottom-0 left-0 w-full h-[72px] bg-white border-t border-slate-200 px-4 flex items-center justify-between z-50 shadow-[0_-4px_20px_rgb(0,0,0,0.05)]">
        <div>
          ...
          <div className="text-[11px] font-bold text-sky-600">{medicine.availability}</div>
        </div>
        <div className="flex gap-2">
          <button className="h-[48px] px-6 bg-white text-slate-900 rounded-xl font-bold text-[14px] shadow-sm flex items-center justify-center gap-2">
            Consult
          </button>
          <button className="h-[48px] px-6 bg-sky-600 text-white rounded-xl font-bold text-[14px] shadow-sm shadow-primary/20 flex items-center justify-center gap-2">
            Buy
          </button>
        </div>
      </div>

...
export default function WalkInPage() {
  ...
  const [tokenId, setTokenId] = useState('S-101');
            ...
            <div className="sticky top-[100px] flex flex-col gap-6">
              ...
              <div className={`bg-white rounded-[24px] shadow-sm border p-6 lg:p-8 flex flex-col items-center text-center transition-colors duration-500 ${isGenerated ? 'border-emerald-200 shadow-emerald-100/50' : 'border-slate-200'}`}>
                <div className="flex items-center justify-between w-full mb-6">
                  <span className="text-[12px] text-slate-500 font-bold uppercase tracking-widest">Live Preview</span>
                ...
                <div className="w-full relative bg-white border-2 border-dashed border-slate-200 rounded-[20px] p-6 lg:p-8 mb-6 shadow-sm">
                  ...
                  <h3 className="text-[18px] font-black text-slate-800 mb-6 text-center">Oxpecker AI Hospital</h3>
                  ...
                  <div className="flex flex-col items-center justify-center mb-6">
                    ...
                    <div className="text-[11px] text-slate-500 font-bold uppercase tracking-widest mb-1">Token Number</div>
                  ...
                  <div className="flex flex-col gap-4 text-left border-t-2 border-dashed border-slate-100 pt-6">
                    <div className="flex justify-between items-end gap-2">
                      <div>
                        <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-0.5">Patient Name</p>
                        <p className="font-bold text-slate-800 text-[15px]">{formData.name || 'Not specified'}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-0.5">Age</p>
                        <p className="font-bold text-slate-800 text-[15px]">{formData.age ? `${formData.age} Y` : '--'}</p>
                      </div>
                    </div>
                    <div>
                      <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-0.5">Phone Number</p>
                      <p className="font-bold text-slate-800 text-[15px]">{phoneDigits.join('') || 'Not specified'}</p>
                    </div>
                    <div>
                      <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-0.5">Assigned Doctor</p>
                    ...
                    <div className="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-100 mt-2">
                      <div>
                        <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-0.5">Payment</p>
                        <p className="font-bold text-slate-700 text-[14px]">{formData.paymentMethod}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-0.5">Discount</p>
                        <p className="font-bold text-slate-700 text-[14px]">{formData.discount ? `${formData.discount}%` : '0%'}</p>
                      </div>
                      <div>
                        <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-0.5">Total Paid</p>
                        <p className="font-black text-indigo-600 text-[16px]">৳{total}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-0.5">Priority</p>
                ...
                <button 
                  disabled={!isGenerated}
                  className={`w-full h-[56px] font-bold text-[15px] rounded-xl flex items-center justify-center gap-2 transition-all ${isGenerated ? 'bg-slate-800 text-white hover:bg-slate-700 shadow-md active:scale-95' : 'bg-slate-50 text-slate-500 border border-slate-200 cursor-not-allowed'}`}
                >

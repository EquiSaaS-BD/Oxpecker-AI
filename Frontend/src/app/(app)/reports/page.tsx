...
export default function ReportsPage(props: any) {
  ...
  const handleDownload = (title: string) => {
    toast.success(`${title} ডাউনলোড শুরু হচ্ছে...`);
  };
      ...
      <div className={`bg-white border-b border-slate-200 py-6 md:py-8 sticky top-0 z-20 shadow-sm px-4 md:px-6 lg:px-12 overflow-hidden relative ${isEmbedded ? '' : 'rounded-t-3xl md:rounded-none'}`}>
        ...
        <div className="max-w-[1200px] mx-auto flex flex-col md:flex-row md:items-end justify-between gap-4 md:gap-6 relative z-10">
          <div className="text-center md:text-left">
            <h1 className="text-[26px] md:text-[34px] font-[900] text-slate-900 leading-tight tracking-tight font-serif drop-shadow-sm">
              My Medical Reports
            </h1>
          ...
          <div className="relative w-full md:w-[350px] shrink-0">
            ...
            <input 
              ...
              className="w-full h-12 pl-10 pr-4 bg-slate-50 border border-slate-200 rounded-[14px] text-[15px] font-medium text-slate-800 placeholder:text-slate-500 focus:outline-none focus:border-[#00C2A8] focus:ring-2 focus:ring-[#00C2A8]/20 transition-all shadow-inner"
            />
          </div>
        </div>
          ...
          {categories.map(cat => (
            <button 
              ...
              className={`px-5 py-2 md:py-2.5 rounded-full text-[13px] md:text-[14px] font-bold whitespace-nowrap transition-all duration-300 ${
                activeTab === cat 
                  ? "bg-[#00C2A8] text-slate-900 shadow-md shadow-[#00C2A8]/20" 
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-[12px] md:text-[13px] font-medium text-slate-500 mt-2 md:mt-3 mb-4">
                      <div className="flex items-center gap-1.5 bg-slate-50 px-2 py-1 rounded-lg border border-slate-100">
                        <Calendar size={14} className="text-slate-500" /> {report.date}
                      </div>
                      <div className="flex items-center gap-1.5 bg-slate-50 px-2 py-1 rounded-lg border border-slate-100">
                        <User size={14} className="text-slate-500" /> {report.doctor}
                      </div>
                    </div>
                  </div>

                  <div className="flex items-center justify-between pt-3 md:pt-4 border-t border-slate-100 mt-auto">
                    <span className="text-[12px] md:text-[13px] font-bold text-slate-500">{report.size} • PDF</span>
                    <div className="flex items-center gap-2">
                      ...
                      <button 
                        ...
                        className="h-9 md:h-10 px-3 md:px-4 bg-white hover:bg-slate-50 text-slate-900 rounded-xl font-[800] text-[13px] shadow-sm transition-colors flex items-center gap-2 cursor-pointer"
                      >
      ...
      <AnimatePresence>
        {selectedReport && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[100] bg-white/60 backdrop-blur-sm flex items-center justify-center p-0 sm:p-6 lg:p-12"
          >
            <motion.div 
              ...
              <div className="h-[72px] md:h-[80px] bg-white border-b border-slate-100 flex items-center justify-between px-4 md:px-8 shrink-0">
                ...
                <div className="flex items-center gap-2 md:gap-4 shrink-0 pl-4">
                  <button 
                    ...
                    className="h-10 px-4 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-[800] text-[13px] transition-colors hidden sm:flex items-center gap-2 cursor-pointer"
                  >
                  ...
                  <div className="space-y-6">
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex justify-between">
                      <div>
                        ...
                        <p className="font-bold text-slate-800">{user?.name || "Patient Profile"}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Referred By</p>
                        <p className="font-bold text-slate-800">{selectedReport.doctor}</p>
                      </div>
                    </div>

                    <div className="h-64 w-full border-2 border-dashed border-slate-200 rounded-xl flex flex-col items-center justify-center text-slate-500 bg-slate-50/50">
                      <FileText size={48} className="mb-4 opacity-50" />
                      <p className="font-medium text-slate-700">Digital Medical Document Preview</p>
                      <p className="text-xs text-slate-400 mt-2">Verified Digital Health Record</p>
                    </div>
                  </div>

...
const DEFAULT_COLUMNS: AwwwardsNavColumn[] = [
  ...
  },
  {
    ...
    links: [
      ...
      { label: "Nutrition & Diet", href: "/nutrition", icon: Apple },
    ],
  },
  {
  ...
  return (
    ...
    >
      ...
      <div ref={navTopRef} className="absolute inset-x-0 top-0 bottom-[64px] hidden p-3 overflow-y-auto scrollbar-thin">
        <div className="flex flex-col sm:flex-row h-full w-full gap-4 rounded-xl border border-slate-200/80 bg-white/60 p-4">
          {columns.map((col, ci) => (
            <div
              key={col.title}
              className={cn(
                "flex flex-1 flex-col gap-1.5",
                ci > 0 && "sm:border-l sm:border-dashed sm:border-slate-200 sm:pl-4 pt-3 sm:pt-0 border-t border-slate-200 sm:border-t-0"
              )}
            >
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      {Icon && <Icon size={15} className={isActive ? "text-sky-600" : "text-slate-500 group-hover:text-slate-600"} />}
      ...
      <div className="absolute inset-x-0 bottom-0 flex h-[64px] gap-1.5 p-2">
        <div
          ...
          className={cn(
            "flex shrink-0 cursor-pointer select-none items-center justify-center gap-1.5 rounded-xl border px-2.5 sm:px-4 text-xs sm:text-[13px] font-bold transition-all",
            "border-slate-200 bg-white/90 text-slate-600 hover:bg-slate-50 hover:text-slate-800 hover:border-slate-300",
        ...
        <div ref={navItemsRef} className="flex min-w-0 flex-[4] items-center gap-1 sm:gap-1.5 overflow-x-hidden">
          {items.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  "flex h-full flex-1 min-w-0 items-center justify-center gap-1 rounded-xl border px-1 sm:px-2 text-center text-[10px] sm:text-[13px] font-bold transition-all shrink-1",
                  isActive
                    ? "border-sky-300 bg-sky-50 text-sky-700 font-extrabold shadow-xs"
                    : "border-slate-200/80 bg-white/70 text-slate-500 hover:border-slate-300 hover:text-slate-700 hover:bg-slate-50/80"
                )}
              >
                {Icon && <Icon size={15} className={cn("shrink-0", isActive ? "text-sky-600" : "text-slate-500")} />}
                <span className="block truncate max-w-[46px] sm:max-w-none text-center">{item.label}</span>
              </Link>
            );
          })}
        </div>

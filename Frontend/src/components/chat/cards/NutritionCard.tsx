...
export function NutritionCard({ data }: { data: NutritionData }) {
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="bg-gradient-to-r from-green-500 to-emerald-600 px-5 py-3.5 flex items-center justify-between">
        <div className="flex items-center gap-2 text-white">
          <Utensils size={18} />
          <span className="font-bold text-[15px]">Nutrition Analysis</span>
        </div>
        {data.mealType && (
          <span className="text-[12px] bg-white/20 text-white px-2.5 py-1 rounded-lg font-medium capitalize">
            {data.mealType}
          </span>
        )}
      </div>
      ...
      <div className="p-5">
        ...
        <div className="grid grid-cols-5 gap-2 mb-4">
          {macros.map((m, i) => (
            <div key={i} className="text-center">
              ...
              <p className="text-[10px] text-slate-400 font-medium">{m.unit}</p>
        ...
        {data.foods && data.foods.length > 0 && (
          <div className="border-t border-slate-100 pt-3">
            <p className="text-[12px] font-bold text-slate-400 uppercase tracking-wider mb-2">Identified Foods</p>
            <div className="space-y-1.5">
              {data.foods.map((food, i) => (
                <div key={i} className="flex items-center justify-between text-[13px]">
                  <span className="text-slate-700 font-medium">{food.name} <span className="text-slate-400">({food.portion})</span></span>

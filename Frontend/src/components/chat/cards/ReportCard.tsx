...
export function ReportCard({ data }: { data: ReportData }) {
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="bg-indigo-50 border-b border-indigo-100 px-5 py-3.5 flex items-center justify-between">
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="text-center bg-slate-50 rounded-xl p-3">
            ...
            <p className="text-[11px] text-slate-500 font-medium">Parameters</p>
          </div>
        ...
        {data.parameters && data.parameters.length > 0 && (
          <div>
            <p className="text-[12px] font-bold text-slate-500 uppercase tracking-wider mb-2">Test Results</p>
            <div className="border border-slate-100 rounded-xl overflow-hidden">
              <div className="grid grid-cols-4 gap-1 bg-slate-50 px-3 py-2 text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                <span>Parameter</span>
              ...
              {data.parameters.slice(0, 8).map((p, i) => (
                <div key={i} className={`grid grid-cols-4 gap-1 px-3 py-2.5 text-[13px] border-t border-slate-50 ${p.status !== 'normal' ? 'bg-rose-50/30' : ''}`}>
                  ...
                  <span className={`text-center font-bold ${p.status !== 'normal' ? 'text-rose-600' : 'text-slate-800'}`}>{p.value} <span className="text-[10px] text-slate-500">{p.unit}</span></span>
                  <span className="text-center text-slate-500 text-[12px]">{p.normalRange}</span>

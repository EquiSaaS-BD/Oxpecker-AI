    ...
    <div className="w-[400px] h-full bg-white border-l border-slate-200 shadow-[-4px_0_24px_rgba(0,0,0,0.02)] flex flex-col absolute right-0 top-0 z-10 transition-transform duration-300 ease-in-out transform">
      ...
      <div className="h-[64px] shrink-0 border-b border-slate-100 flex items-center justify-between px-6">
        ...
        <div className="flex items-center gap-2">
          <button className="w-8 h-8 flex items-center justify-center text-slate-500 hover:text-[#2F80ED] hover:bg-blue-50 rounded-lg transition-colors">
            <Printer size={16} />
          </button>
          <button className="w-8 h-8 flex items-center justify-center text-slate-500 hover:text-[#2F80ED] hover:bg-blue-50 rounded-lg transition-colors">
            <Download size={16} />
          ...
          <button 
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center text-slate-500 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"
          >
            <X size={18} />
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6 flex flex-col gap-6">
        ...
        <div className="w-full bg-[#F1F5F9] rounded-[12px] border border-slate-200 overflow-hidden relative shadow-sm" style={{ aspectRatio: '1 / 1.4' }}>
          <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-500 gap-3 p-6 text-center">
            ...
            <div>
              ...
              <p className="text-[12px] text-slate-500">The scanned diagnostic report will appear here in high resolution.</p>
            </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <textarea 
            ...
            className="w-full h-[120px] p-3 bg-white border border-slate-200 rounded-[12px] text-[14px] text-slate-700 placeholder:text-slate-500 resize-none outline-none focus:border-[#2F80ED] focus:ring-1 focus:ring-[#2F80ED]/20 transition-all shadow-sm"
          />
        </div>

        {/* Action Button */}
        <button className="w-full h-[48px] bg-[#6DDA6E] hover:bg-[#5bc95c] text-slate-900 font-bold text-[14px] rounded-[12px] transition-colors flex items-center justify-center gap-2 shadow-sm">

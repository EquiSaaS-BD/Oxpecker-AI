...
export function TestReportViewer() {
  ...
  const renderViewer = () => {
    return (
      <div className="fixed inset-0 z-[99999] sm:static sm:z-auto flex-1 flex flex-col bg-slate-800 overflow-hidden sm:h-full">
        <div className="bg-white border-b border-slate-200 p-4 sm:px-6 sm:py-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shrink-0 shadow-sm z-10 pt-safe-top">
          <div className="flex items-center gap-3 sm:gap-4 w-full sm:w-auto">
            <button 
              ...
              className="w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center bg-slate-800 text-slate-300 hover:text-white rounded-full transition-colors shrink-0"
            >
          ...
          <div className="flex items-center justify-between sm:justify-end gap-2 sm:gap-4 w-full sm:w-auto">
            ...
            <div className="flex gap-1.5 sm:gap-2 shrink-0">
              ...
              <button 
                ...
                className={`w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-lg transition-colors ${isDrawingMode ? 'bg-red-100 text-red-600 ring-2 ring-red-400' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-50'}`}
                title="Mark / Annotate"
              >
                <PenTool size={16} className="sm:w-[18px] sm:h-[18px]" />
              </button>
              <button className="w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center text-slate-500 hover:text-slate-800 hover:bg-slate-50 rounded-lg transition-colors">
          ...
          <div className="sticky top-0 left-0 w-full flex justify-center z-40 px-4 pointer-events-none mb-[-56px]">
            <div className="bg-white/60 backdrop-blur-md border border-white/60 shadow-[0_8px_32px_rgba(0,0,0,0.1)] rounded-full px-1.5 py-1.5 flex items-center gap-1 pointer-events-auto">
              <button 
                ...
                className="w-10 h-10 rounded-full flex items-center justify-center text-slate-800 hover:text-sky-400 bg-white/40 hover:bg-white/80 active:scale-95 transition-all"
              >
              ...
              <div className="absolute inset-0 p-8 sm:p-12 flex flex-col pointer-events-none" style={{ transform: `scale(${zoom})`, transformOrigin: 'top left', width: '794px', height: '1123px' }}>
                <div className="flex justify-between items-start border-b-2 border-slate-200 pb-6 mb-8">
                  ...
                  <div className="text-right">
                    ...
                    <p className="font-bold text-slate-800 font-mono bg-slate-800 px-2 py-0.5 rounded-md mt-2 inline-block">
                      ID: {mrn}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex-1 flex flex-col items-center justify-center opacity-40">
                  ...
                  <p className="text-xl font-bold text-slate-500">Mock Data / Visual Demo</p>
                  <p className="text-sm text-slate-500 mt-2">Actual PDF rendering goes here</p>
                </div>
              </div>
        ...
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-4 bg-white/90 backdrop-blur-md px-4 sm:px-6 py-2 sm:py-3 rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.12)] border border-slate-200/50 pb-safe">
          <button 
            onClick={() => setCurrentPage(p => Math.max(1, p - 1))}
            disabled={currentPage === 1}
            className="flex items-center gap-1 sm:gap-2 text-[13px] sm:text-[14px] font-bold text-slate-700 hover:text-sky-400 disabled:opacity-40 transition-colors"
          >
        ...
        <div className="bg-white border-b border-slate-200 p-4 sm:p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shrink-0 transition-all">
          <div>
            ...
            <p className="text-[12px] sm:text-sm text-slate-500 mt-1 sm:mt-1.5 leading-relaxed">
              Reports are automatically forwarded by the Lab using Patient ID: <br className="sm:hidden" /><span className="font-bold font-mono text-slate-700 bg-slate-800 px-2 py-0.5 rounded-md mt-1 sm:mt-0 inline-block">{mrn}</span>
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 sm:p-6 pb-24">
          {isLoading ? (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 animate-in fade-in duration-500">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="bg-white p-5 rounded-2xl border border-slate-200 flex flex-col shadow-sm">
                ...
                <div key={idx} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 flex flex-col hover:border-[#2F80ED]/30 transition-colors group cursor-pointer" onClick={() => handleOpenReport(file)}>
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-blue-50 text-sky-400 rounded-xl flex items-center justify-center shrink-0">
                        <FileText size={24} />
                      </div>
                      <div>

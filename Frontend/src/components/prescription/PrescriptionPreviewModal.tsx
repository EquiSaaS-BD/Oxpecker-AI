...
export function PrescriptionPreviewModal({ isOpen, onClose }: PrescriptionPreviewModalProps) {
  ...
  const handleDownloadJPG = async () => {
    ...
    const originalOpacity = printRef.current.style.opacity;
    const originalTop = printRef.current.style.top;
    const originalPosition = printRef.current.style.position;
    const originalZIndex = printRef.current.style.zIndex;
    ...
    try {
      ...
      link.download = `Prescription_${new Date().getTime()}.jpg`;
      link.href = dataUrl;
      link.click();
      toast.success("Prescription downloaded as JPG!");
    } catch (err) {
      console.error("Error generating JPG:", err);
      toast.error("Failed to download JPG");
    } finally {
      printRef.current.style.opacity = originalOpacity;
      printRef.current.style.top = originalTop;
      printRef.current.style.position = originalPosition;
      printRef.current.style.zIndex = originalZIndex;
      ...
      <div className="relative z-10 flex flex-col flex-1 h-full">
        ...
        {data.theme !== 'custom' && (
          <div className={`flex justify-between items-start pb-6 mb-4 ${
            ...
            data.theme === 'classic' ? 'border-b-[3px] border-slate-200 double' : 
...
Medicines: ${data.medicines.map(m => (m as any).brandName || m.name).join(', ') || 'None'}
Tests: ${data.investigationsList?.join(', ') || 'None'}`;

                    return (
                      <>
                        <div className="text-right">
                          <p className={`text-sm font-bold ${data.theme === 'classic' ? 'text-slate-900' : 'text-slate-800'}`}>Oxpecker AI Hospital</p>
        ...
        <div className={`flex justify-between items-center px-4 py-3 mb-8 ${
          ...
          <div>
            ...
            <span className="ml-2 text-sm font-bold text-slate-800" suppressHydrationWarning>{new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
          </div>
        </div>
                ...
                <div className={`flex flex-col ${pageMedicines.length > 7 ? 'gap-2' : pageMedicines.length > 4 ? 'gap-4' : 'gap-6'}`}>
                  {pageMedicines.map((med: any, index: number) => {
                    ...
                    <div key={index} className={`flex ${pageMedicines.length > 7 ? 'gap-2' : 'gap-4'}`}>
                      <span className={`font-bold text-slate-500 mt-0.5 ${pageMedicines.length > 7 ? 'text-[11px]' : pageMedicines.length > 4 ? 'text-xs' : 'text-sm'}`}>{globalIndex + 1}.</span>
                      <div className="flex-1">
        ...
        <div className="mt-auto pt-6 border-t border-slate-200 flex flex-col gap-4">
          <div className="flex justify-end w-full">
            <div className="text-center">
              <div className="w-40 border-b border-slate-200 mb-2"></div>
          ...
          {totalPages > 1 && (
            <div className="flex justify-between items-center w-full pt-4">
              {!isPrintMode ? (
                ...
                <div className="mb-2">
                  <span className="text-[11px] font-bold text-slate-500 bg-slate-50 border border-slate-100 px-3 py-1 rounded-full shadow-sm">
                    Page {pageIndex + 1} of {totalPages}
                  </span>
                </div>
              )}
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-start md:items-center justify-center p-0 md:p-6 bg-white/40 backdrop-blur-sm">
        <motion.div
          initial={{ opacity: 0, scale: 0.95, y: 10 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 0.95, y: 10 }}
          className="bg-slate-50 w-full max-w-5xl h-[100dvh] md:h-full md:max-h-[90vh] rounded-none md:rounded-2xl shadow-2xl flex flex-col md:flex-row overflow-hidden"
        >
          {/* Main Content Area */}
          <div className="flex-1 flex flex-col h-full overflow-hidden relative">
            {/* Header */}
            <div className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between shrink-0">
              <h2 className="text-lg font-bold text-slate-700 flex items-center gap-4">
                {activeView === 'theme-settings' ? 'Theme Settings' : 'Prescription Preview'}
              </h2>
              
              <div className="flex items-center gap-4">
                {activeView === 'preview' && (
                  <div className="flex items-center gap-1 bg-slate-50 p-1 rounded-lg border border-slate-200 hidden sm:flex">
                    <button 
                      onClick={() => setZoom(z => Math.max(0.25, z - 0.25))} 
                      className="p-1.5 text-slate-500 hover:text-slate-700 hover:bg-slate-50 rounded shadow-none transition-colors active:scale-95"
                      title="Zoom Out"
                    >
                      <ZoomOut size={16}/>
                    </button>
                ...
                <button onClick={onClose} className="p-2 text-slate-500 hover:text-slate-700 hover:bg-slate-50 rounded-xl transition-colors">
                  <X size={20} />
                </button>
              </div>
            </div>
            ...
            {activeView === 'theme-settings' ? (
              <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-slate-50 flex justify-center">
                <div className="w-full max-w-5xl bg-white rounded-2xl shadow-none border border-slate-200 p-6 md:p-8 flex flex-col gap-10 h-fit">
                  
                  {/* Default Themes */}
                  <div>
                    <h3 className="text-lg font-bold text-slate-700 mb-4">Default Themes</h3>
                      ...
                      {data.customThemeImage ? (
                        ...
                        >
                          <div className="w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center mb-2 group-hover:bg-white transition-colors">
                            <ImagePlus size={18} className="text-slate-500 group-hover:text-[#2F80ED]" />
                          </div>
              ...
              <div className="flex-1 relative flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-auto custom-scrollbar bg-slate-50 flex flex-col relative touch-pan-x touch-pan-y">
                  ...
                  <div className="sm:hidden sticky top-4 left-0 w-full flex justify-center z-40 px-4 pointer-events-none mb-[-56px]">
                    <div className="bg-white/20 backdrop-blur-md border border-white/40 shadow-[0_8px_32px_rgba(0,0,0,0.1)] rounded-full px-1.5 py-1.5 flex items-center gap-1 pointer-events-auto">
                      <button 
                        onClick={() => setZoom(z => Math.max(0.25, z - 0.25))} 
                        className="w-10 h-10 rounded-full flex items-center justify-center text-slate-700 hover:text-sky-400 bg-slate-800/50 hover:bg-slate-700/50 active:scale-95 transition-all"
                      >
                        <ZoomOut size={18}/>
                      </button>
          ...
          <div className="w-full md:w-[260px] bg-white border-t md:border-t-0 md:border-l border-slate-200 shrink-0 flex flex-row md:flex-col p-4 md:p-5 gap-3 z-20 justify-center md:justify-start">
            <h3 className="hidden md:block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">Actions</h3>
            
            {activeView === 'preview' ? (
              <>
                <button 
                  onClick={() => setActiveView('theme-settings')}
                  className="flex flex-1 md:flex-none items-center justify-center md:justify-start gap-3 text-[13px] font-bold text-slate-600 bg-slate-800 border border-slate-300 px-0 md:px-4 py-3.5 rounded-xl hover:bg-slate-700 transition-colors shrink-0 group"
                  title="Theme Settings"
                >
                  <Palette size={20} className="text-sky-400 group-hover:scale-110 transition-transform md:w-[18px] md:h-[18px]" /> 
                ...
                <button 
                  ...
                  className="flex flex-[2] md:flex-none items-center justify-center md:justify-start gap-2 md:gap-3 text-[13px] font-bold text-white bg-sky-600 hover:bg-sky-500 px-0 md:px-4 py-3.5 rounded-xl shadow-sm shadow-sky-500/20 transition-all active:scale-95 disabled:opacity-70 disabled:cursor-wait shrink-0 group"
                  title="Download PDF"
                >
                ...
                <button 
                  ...
                  className="flex flex-[2] md:flex-none items-center justify-center md:justify-start gap-2 md:gap-3 text-[13px] font-bold text-white bg-emerald-600 hover:bg-emerald-500 px-0 md:px-4 py-3.5 rounded-xl shadow-sm shadow-emerald-600/20 transition-all active:scale-95 disabled:opacity-70 disabled:cursor-wait shrink-0 group"
                  title="Download JPG"
                >
                  {isDownloadingJPG ? <Loader2 size={20} className="animate-spin md:w-[18px] md:h-[18px]" /> : <Download size={20} className="group-hover:-translate-y-0.5 transition-transform md:w-[18px] md:h-[18px]" />}
                  <span className="hidden md:inline">{isDownloadingJPG ? "Generating..." : "Download JPG"}</span>
                  <span className="md:hidden ml-1">{isDownloadingJPG ? "..." : "JPG"}</span>
                </button>
              </>
            ) : (
              <button 

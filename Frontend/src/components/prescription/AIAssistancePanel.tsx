...
import { Sparkles, Send, Mic, Copy, Check, X, ShieldAlert, BookOpen, Stethoscope, FileText, Activity, AlertCircle, PlusCircle, ArrowRight, ArrowUp, Brain, Pill, ClipboardList, Camera, RefreshCw, Edit3, FileSearch, Clipboard, Download, MicOff, History } from "lucide-react";
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-5 flex flex-col gap-4 bg-transparent min-h-0">
        {messages.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center mt-4">
            <div className="w-14 h-14 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl shadow-md flex items-center justify-center mb-4 relative">
      ...
      <div className="p-4 pb-8 xl:pb-4 bg-white border-t border-slate-200 mt-auto shrink-0 z-10 sticky bottom-0" style={{ paddingBottom: 'max(env(safe-area-inset-bottom), 1rem)' }}>
        ...
        {messages.length > 0 && (
          <div className="flex gap-1.5 mb-2 overflow-x-auto pb-1 scrollbar-none">
            <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-1 bg-purple-50 text-purple-600 text-[11px] font-bold px-2.5 py-1 rounded-lg shrink-0 hover:bg-purple-100 transition-colors">

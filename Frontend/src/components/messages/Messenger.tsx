            ...
            >
              ...
              <div className="flex-1 min-w-0 flex flex-col justify-center">
                <div className="flex items-center justify-between mb-0.5">
                  ...
                  <span className={`text-[11px] font-medium shrink-0 ml-2 ${chat.unread > 0 ? 'text-sky-600' : 'text-slate-500'}`}>
                    {chat.time}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  ...
                  {chat.unread > 0 && (
                    <span className="w-5 h-5 bg-sky-600 text-white rounded-full flex items-center justify-center text-[10px] font-bold shrink-0">
                      {chat.unread}
                    </span>
                  )}
                </div>
      ...
      <div className={`flex-1 flex-col h-full bg-[#F8FAFC] ${showChatOnMobile ? 'flex' : 'hidden md:flex'}`}>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-6 flex flex-col gap-4 md:gap-6">
          ...
          <div className="flex flex-col items-end gap-1 w-full mt-2 md:mt-4">
            <div className="flex items-end gap-2 max-w-[90%] md:max-w-[70%] flex-row-reverse">
              <div className="bg-sky-600 text-white p-3 md:p-[12px_16px] rounded-[16px] rounded-br-none shadow-sm text-[14px] md:text-[15px] leading-[1.5]">
                Sure Dr. Aminul, I'm checking the report right now.
              </div>
            </div>
            <span className="text-[11px] text-slate-500 font-medium mr-1 md:mr-2">10:32 AM</span>
          </div>
        </div>
        ...
        <div className="min-h-[70px] md:min-h-[80px] bg-white border-t border-slate-200 p-3 md:p-[16px_24px] flex items-center shrink-0">
          <div className="flex-1 flex items-center bg-slate-50 border border-slate-200 rounded-[24px] pl-1 md:pl-2 pr-1 md:pr-2">
            <button className="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center text-slate-500 hover:text-slate-600 rounded-full transition-colors shrink-0">
              <Smile size={18} className="md:w-5 md:h-5" />
            </button>
            <input 
              type="text" 
              placeholder="Type your message..."
              className="flex-1 h-[40px] md:h-[48px] bg-transparent border-none outline-none text-[14px] md:text-[15px] text-slate-700 placeholder:text-slate-500 px-2 min-w-0"
            />
            <button className="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center text-slate-500 hover:text-slate-600 rounded-full transition-colors shrink-0 mr-1">
              <Paperclip size={18} className="md:w-5 md:h-5" />
            </button>
            <button className="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center bg-sky-600 text-white hover:bg-sky-700 rounded-full transition-colors shrink-0 shadow-sm">

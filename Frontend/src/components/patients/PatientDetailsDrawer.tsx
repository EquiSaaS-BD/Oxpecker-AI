      ...
      {isOpen && patient && (
        <>
          ...
          <motion.div
            ...
            exit={{ opacity: 0 }}
            onClick={onClose}
            className="fixed inset-0 z-[99998] bg-white/20 backdrop-blur-sm"
          />
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
              ...
              <div>
                ...
                <div className="bg-slate-50 border border-slate-100 p-4 rounded-xl space-y-3">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-sm font-bold text-slate-800" suppressHydrationWarning>{new Date(patient.date).toLocaleDateString('en-GB')}</span>
                  </div>
                </div>
              </div>
              ...
              <div>
                <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider mb-3 flex items-center gap-2">
                  <Clock size={16} className="text-slate-500" /> Past Prescriptions
                </h3>
                      ...
                      <div key={idx} className="bg-white border border-slate-200 p-4 rounded-xl hover:border-primary/40 transition-colors group">
                        <div className="flex justify-between items-start mb-2">
                          <div className="flex items-center gap-2">
                            ...
                            <h4 className="font-bold text-slate-800 text-sm" suppressHydrationWarning>{new Date(rx.date || new Date()).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}</h4>
                          </div>
                        ...
                        {rx.medicines && rx.medicines.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-2">
                            ...
                            {rx.medicines.length > 3 && (
                              <span className="text-[11px] text-slate-500 font-medium px-1 py-0.5">+{rx.medicines.length - 3} more</span>
                            )}
                          </div>
                        )}
                      </div>

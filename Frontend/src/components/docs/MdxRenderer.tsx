...
export function MdxRenderer({ content }: MdxRendererProps) {
  ...
  return (
    <>
      ...
      />
      <div className="prose prose-slate dark:prose-invert max-w-none prose-headings:font-bold prose-headings:tracking-tight prose-a:font-medium prose-a:text-[#00C2A8] hover:prose-a:text-[#6DDA6E] prose-a:underline-offset-4 prose-p:text-[16px] prose-p:leading-[1.8] prose-p:text-slate-600 dark:prose-p:text-slate-300 prose-li:text-slate-600 dark:prose-li:text-slate-300 prose-strong:text-slate-900 dark:prose-strong:text-slate-100 prose-blockquote:border-l-[#6DDA6E] prose-blockquote:bg-slate-50 dark:prose-blockquote:bg-slate-900/50 prose-blockquote:px-6 prose-blockquote:py-2 prose-blockquote:rounded-r-xl prose-blockquote:not-italic prose-blockquote:text-slate-700 dark:prose-blockquote:text-slate-300 prose-img:rounded-2xl prose-img:shadow-xl prose-img:border prose-img:border-slate-200 dark:prose-img:border-slate-800">
        <ReactMarkdown
          components={{
            code({ node, inline, className, children, ...props }: any) {
              ...
              if (!inline && language === 'mermaid') {
                return (
                  <div className="relative group my-10 rounded-[16px] overflow-hidden border border-slate-200 dark:border-slate-800/80 shadow-lg bg-[#0F172A]">
            ...
            return !inline && match ? (
              ...
              <code className="bg-slate-100 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 px-1.5 py-0.5 rounded-md text-[14px] font-medium text-slate-800 dark:text-slate-200" {...props}>
                {children}
              </code>
            );
          },
          h1: ({node, ...props}) => <h1 className="text-[42px] sm:text-[48px] tracking-tight font-extrabold mb-8 text-slate-900 dark:text-white" {...props} />,
          h2: ({node, ...props}) => {
             const id = props.children?.toString().toLowerCase().replace(/\s+/g, '-');
             return <h2 id={id} className="text-[32px] font-bold mt-[80px] scroll-mt-28 border-b border-slate-200 dark:border-slate-800/60 pb-4 text-slate-900 dark:text-white group relative" {...props}>
               {props.children}
               <a href={`#${id}`} className="absolute -left-6 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 text-slate-400 hover:text-[#6DDA6E] transition-opacity">#</a>
             </h2>;
          },
          h3: ({node, ...props}) => <h3 className="text-[24px] font-semibold mt-12 text-slate-900 dark:text-slate-100" {...props} />,
          table: ({node, ...props}) => (
             <div className="overflow-x-auto my-10 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm">
               <table className="w-full text-left border-collapse text-sm" {...props} />
             </div>
          ),
          th: ({node, ...props}) => <th className="bg-slate-50 dark:bg-slate-900/50 p-4 border-b border-slate-200 dark:border-slate-800 font-semibold text-slate-900 dark:text-slate-200" {...props} />,
          td: ({node, ...props}) => <td className="p-4 border-b border-slate-100 dark:border-slate-800/30 text-slate-600 dark:text-slate-300" {...props} />,

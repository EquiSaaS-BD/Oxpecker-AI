...
export function MegaMenuNavbar({
  ...
  const SERVICES_FEATURES: MegaMenuItem[] = [
    ...
    {
      title: t("Food Calorie Scanner", "ফুড ক্যালোরি ও ডায়েট"),
      description: t("Scan food items to analyze calories, nutrition values, and daily health goals.", "খাবারের ছবি তুলে ক্যালোরি ও পুষ্টি উপাদান বিশ্লেষণ এবং স্বাস্থ্য লক্ষ্য পূরণ করুন।"),
      href: "/nutrition",
      icon: Apple,
  ...
  return (
    <header
      ...
      ref={navRef}
      className={cn(
        "fixed top-0 left-0 right-0 z-[100] w-full border-b border-white/40 glass-panel transition-all duration-300",
        className
      )}
      onMouseLeave={(event) => {
        setOpenMenu(null);
    ...
    >
      <div className="mx-auto max-w-7xl px-4 md:px-6">
        <div className="flex h-16 lg:h-20 items-center justify-between">
          ...
          {/* Left Brand Logo */}
          <Link href="/" className="relative z-10 flex shrink-0 items-center max-w-[125px] sm:max-w-none">
            <Image
          ...
          <nav aria-label="Primary navigation" className="hidden items-center md:flex">
            <ul className="flex items-center gap-1 lg:gap-1.5 bg-slate-100/70 backdrop-blur-md px-1.5 lg:px-2 py-1 lg:py-1.5 rounded-full border border-slate-200/60 shadow-xs">
              ...
              <li className="relative" onMouseEnter={() => setOpenMenu("services")}>
                <button
                  ...
                  className={cn(
                    "flex items-center gap-1 rounded-full px-2.5 lg:px-4 py-1.5 lg:py-2 text-xs lg:text-sm font-semibold transition-colors",
                ...
                <div
                  className={cn(
                    "absolute left-0 top-full z-50 pt-2 transition-all duration-200 ease-out origin-top-left",
                    openMenu === "services"
              ...
              <li>
                <Link
                  href="/doctors"
                  className={cn(
                    "inline-flex rounded-full px-2.5 lg:px-4 py-1.5 lg:py-2 text-xs lg:text-sm font-semibold transition-colors",
              ...
              <li className="relative" onMouseEnter={() => setOpenMenu("resources")}>
                ...
                <div
                  className={cn(
                    "absolute left-1/2 -translate-x-1/2 top-full z-50 pt-2 transition-all duration-200 ease-out origin-top",
                    openMenu === "resources"
                      ? "visible scale-100 opacity-100"
                      : "invisible scale-95 opacity-0 pointer-events-none"
                  )}
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            {!isAuthenticated ? (
              <>
                <Link href="/login" className="group relative overflow-hidden px-4 sm:px-5 py-2 sm:py-2.5 text-xs sm:text-sm font-semibold text-slate-600 hover:text-slate-900 transition-all duration-300 hidden sm:block hover:bg-slate-100 rounded-xl sm:rounded-2xl hover:shadow-[inset_0_1px_4px_rgba(0,0,0,0.05)] active:scale-95">
                    <span className="relative z-10">{t("Log In", "লগ ইন")}</span>
                    <div className="absolute inset-0 h-full w-[150%] bg-gradient-to-r from-transparent via-slate-300/40 to-transparent -translate-x-[150%] skew-x-[-25deg] group-hover:translate-x-[100%] transition-transform duration-700 ease-in-out" />
                  </Link>
                <Link href="/register" className="group relative overflow-hidden px-3.5 sm:px-6 py-2 sm:py-2.5 text-xs sm:text-sm font-bold bg-slate-900 hover:bg-slate-800 text-white rounded-xl sm:rounded-2xl shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-300 active:scale-[0.96] flex items-center justify-center shrink-0">
                    <span className="relative z-10">{t("Get Started", "শুরু করুন")}</span>
                    <div className="absolute inset-0 h-full w-[150%] bg-gradient-to-r from-transparent via-white/30 to-transparent -translate-x-[150%] skew-x-[-25deg] group-hover:translate-x-[100%] transition-transform duration-700 ease-in-out" />
                  </Link>
              </>
            ) : (
              <Link
                ...
                className="px-3.5 sm:px-5 py-2 sm:py-2.5 text-xs sm:text-sm font-bold bg-[#0ea5e9] hover:bg-sky-600 text-white rounded-xl shadow-md transition-all active:scale-[0.98] shrink-0"
              >

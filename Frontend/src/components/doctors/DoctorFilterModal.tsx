...
export function DoctorFilterModal({ isOpen, onClose }: DoctorFilterModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex justify-end">
      {/* Backdrop */}
      <div 
        className={`absolute inset-0 bg-white/30 backdrop-blur-[2px] transition-opacity duration-300 ${visible ? "opacity-100" : "opacity-0"}`} 
        onClick={handleClose} 
      />
      ...
      <div className={`relative w-full max-w-[420px] h-full bg-white shadow-2xl flex flex-col transition-transform duration-300 ease-out ${visible ? "translate-x-0" : "translate-x-full"}`}>
        ...
        <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between shrink-0">
          ...
          <button onClick={handleClose} className="w-8 h-8 flex items-center justify-center text-slate-500 hover:text-slate-700 hover:bg-slate-100 rounded-lg transition-colors">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="flex-1 p-6 overflow-y-auto scrollbar-thin space-y-7">
          ...
          {/* Speciality */}
          <div>
            <h3 className="text-[13px] font-bold text-slate-500 uppercase tracking-wider mb-3">Speciality</h3>
          ...
          {/* Consultation Type */}
          <div>
            <h3 className="text-[13px] font-bold text-slate-500 uppercase tracking-wider mb-3">Consultation Type</h3>
            <div className="flex flex-wrap gap-2">
          ...
          {/* Availability */}
          <div>
            <h3 className="text-[13px] font-bold text-slate-500 uppercase tracking-wider mb-3">Availability</h3>
            <div className="flex flex-wrap gap-2">
          ...
          <div>
            <div className="flex justify-between items-center mb-3">
              <h3 className="text-[13px] font-bold text-slate-500 uppercase tracking-wider">Consultation Fee</h3>

  ...
  return (
    <div className="w-full bg-white border border-slate-100 rounded-3xl p-6 shadow-lg shadow-slate-200/40 group hover:border-primary/20 hover:-translate-y-1.5 hover:shadow-[0_20px_40px_-15px_rgba(0,61,155,0.12)] transition-all duration-300">
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        <div className="flex items-center gap-1 bg-amber-50 text-amber-700 border border-amber-200/80 px-2.5 py-1 rounded-lg text-xs font-bold">
      ...
      <div className="flex items-center gap-2 mb-1">
        ...
        {doctor.verified && <BadgeCheck size={20} className="text-sky-600 fill-sky-50 shrink-0" />}
      </div>

      {/* Details with Icons */}
      <div className="space-y-2.5 mt-4 mb-6">
        <div className="flex items-center gap-2.5 text-slate-500 text-sm">
          <Stethoscope size={16} className="text-sky-600 shrink-0" />
          <span className="truncate font-medium">{doctor.specialty}</span>
        </div>
        <div className="flex items-center gap-2.5 text-slate-500 text-sm">
          <UserCircle2 size={16} className="text-sky-600 shrink-0" />
      ...
      <Link 
        ...
        className="w-full bg-sky-50 border border-sky-200 hover:bg-sky-600 hover:text-white text-sky-700 font-bold text-sm py-3 rounded-xl flex items-center justify-center gap-2 transition-all duration-300 group-hover:shadow-md"
      >
        <Eye size={16} />
        View Profile
      </Link>
